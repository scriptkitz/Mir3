			...
			switch (wParam)
			{
				...
				case IDOK:
				{
					...
					if (lstrlen(szTitle))
					{
						...
						_stprintf_s(szPrivateIP, _T("%ld.%ld.%ld.%ld"),
							FIRST_IPADDRESS(dwPrivateIP), SECOND_IPADDRESS(dwPrivateIP),
							THIRD_IPADDRESS(dwPrivateIP), FOURTH_IPADDRESS(dwPrivateIP));

						TCHAR	szPublicIP[20];
						_stprintf_s(szPublicIP, _T("%ld.%ld.%ld.%ld"),
							FIRST_IPADDRESS(dwPublicIP), SECOND_IPADDRESS(dwPublicIP),
							THIRD_IPADDRESS(dwPublicIP), FOURTH_IPADDRESS(dwPublicIP));

						jRegSetKey(szKey, _TEXT("PrivateIP"), REG_SZ, (LPBYTE)szPrivateIP, _tcsclen(szPrivateIP) * sizeof(TCHAR));
						jRegSetKey(szKey, _TEXT("PublicIP"), REG_SZ, (LPBYTE)szPublicIP, _tcsclen(szPublicIP) * sizeof(TCHAR));
						...
						if (SendMessage(GetDlgItem(hWndDlg, IDC_IPADDRESS3), IPM_ISBLANK, (WPARAM)0, (LPARAM)0L) == 0)
						{
							...
							GetWindowText(GetDlgItem(hWndDlg, IDC_PORT), szPort, sizeof(szPort)/sizeof(TCHAR));
